...
print(f"🔑 Loaded OpenAI key: {openai.api_key[:10]}...")
...
async def generate_fallback_response(refined_query: str, original_query: str) -> str:
    ...
    try:
        ...
        # ⚠️ General fallback
        print(f"[Fallback] ⚠️ General fallback triggered for query: {refined_query}", flush=True)
        general_prompt = (
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            print(f"\n[WebSocket] Original Query: {received}", flush=True)
            refined = await refine_query(received)
            refined_lower = refined.lower()
